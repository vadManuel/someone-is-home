...
Use `UpdateAsync` over `SetAsync` when multiple servers could write the same key. `UpdateAsync` reads the current value before writing, prepassage use overwrites from racing servers. `SetAsync` is faster but unsafe for concurrent access.
