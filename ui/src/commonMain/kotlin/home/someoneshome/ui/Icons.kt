 ...
 *
 * **Parsed rather than re-drawn.** Re-expressing a path as a sequence of `lineTo` calls is a
 * transcription step with no reviewer — a mistyped coordinate produces an icon that is subtly
 * wrong and looks deliberate. Keeping the `d` strings means each icon can be diffed against the
 * source character for character.
 *
 * Every icon is stroked, never filled, at 1.4–1.5 units on a 20-unit grid. That is the whole
 * reason the set reads as one family, and it is also the dark-field rule doing its work: an
 * outline lights a fraction of the pixels a solid glyph would.
 */
object PanelIcons {
    ...
    val CameraLens = Triple(10f, 10.5f, 2.6f)
...
/**
 * Draws one of [PanelIcons]' path sets at [size], stroked in [color].
 *
 * The paths are parsed once per icon shape and cached against the path data, because parsing on
 * every recomposition would allocate on a screen that is on-camera for the whole round — and this
 * app has a measured whole-app allocation budget to stay inside.
 */
@Composable
fun PanelIcon(
    paths: List<String>,
 ...
 * intensity with no tap target at all, because an app that responded when tapped and then refused
 * would be a channel — the refusal itself carries information about what the device thinks you
 * are allowed to do.
 */
@Composable
fun SpringTile(
    ...
    Column(
        ...
        Box(
            Modifier.size(30.u).border(1.u, border),
