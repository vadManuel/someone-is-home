 ...
 *
 * `max(1, ⌊seats/6⌋)` to `⌈seats/4⌉` — 6 players → 1–2, 8 → 1–2, 12 → 2–3, 16 → 2–4. **Playtest
 * owns these, as it owns the 7.** They are written here once so that the lobby, the host's
 * settings and whatever arms the round cannot each hold a different opinion; they are not a
 * decision this layer is making. Player count is virtually unlimited by design intent — the
 * engineering posture is *designed for 6–10, no hard cap built*.
 */
object InsiderBand {
