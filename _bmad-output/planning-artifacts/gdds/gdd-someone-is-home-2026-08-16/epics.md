...
| ~~**1.7a**~~ | **DONE 2026-08-17 — PASSED.** Shader stalls are a non-issue: the first blackout after a cold launch drew in 3.3–5.6 ms, inside one 8.335 ms frame, and 200 no-pre-warm trials produced no late frame. **The pre-warm mitigation is therefore not needed and should not be built.** Renderer-idle was also cleared — 3–10 s between blackouts costs 0.1 ms |
| ~~**1.7b**~~ | **DONE 2026-08-17 — PASSED, WITH A BUDGET.** The collector is confirmed as the cause of late blackouts, but only above a threshold. Clean at **0.54 MB/s** across 55 200 blackouts; **0.36% miss a frame at 3.00 MB/s**, every one with a collection in its window. **Flutter fallback not taken.** ⚠️ **The mitigation named here was aimed at the wrong layer — see 1.7c** |
| **1.7c** | **REPLACES 1.7b's mitigation. A total allocation-rate budget for the whole app, asserted continuously: ~0.5 MB/s.** The allocation that drives these collections is on the BLE, motion, effect and recording threads — **a blackout-path assertion stays green through every failure that was actually observed.** Keep the blackout-path assertion too; it is cheap and guards a smaller, different failure (a future commit quietly adding allocation to the draw path), but **it is not the guard that matters** and it still has no measured threshold |
...
- ~~**THE STACK GATE (story 1.7)**~~ — **met 2026-08-17.** 55 200 blackouts at or below 0.54 MB/s produced **one** late frame, and it had no collection in its window and no main-thread stall: OS scheduling, ~1 in 55 000, irreducible by any engine choice. Full evidence in `spike-stackgate/FINDINGS.md`.

> **✅ Executed 2026-08-17. Both warnings below were correct, and the second one was needed twice.** The first spike build reported a clean PASS while its pressure generator was contributing 14% on top of the app's own allocation floor — pressure ON and OFF were statistically identical, exactly the meaningless pass warned about. **Four separate instrument bugs each produced a plausible pass in the flattering direction**, including a headline metric that read 0 for every provably-late trial. **Generalise this to E0's performance instrumentation (G4): no performance number is believed until a second, independent source agrees with it.**
>
